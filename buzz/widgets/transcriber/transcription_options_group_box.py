...
from PyQt6.QtWidgets import QGroupBox, QWidget, QFormLayout, QComboBox, QLabel, QHBoxLayout, QLineEdit
...
class TranscriptionOptionsGroupBox(QGroupBox):
    ...
    def __init__(
        ...
        self.ollama_api_url_edit = QLineEdit(self)
        self.ollama_api_url_edit.setPlaceholderText("http://localhost:11434")
        default_url = self.settings.value(
            key=Settings.Key.OLLAMA_API_URL, 
            default_value="http://localhost:11434"
        )
        self.ollama_api_url_edit.setText(default_url)
        self.ollama_api_url_edit.textChanged.connect(
            self.on_ollama_api_url_changed
        )
        
        # Ollama model name input
        self.ollama_model_edit = QLineEdit(self)
        self.ollama_model_edit.setPlaceholderText("whisper")
        default_model = self.settings.value(
            key=Settings.Key.OLLAMA_MODEL, 
            default_value="whisper"
        )
        self.ollama_model_edit.setText(default_model)
        self.ollama_model_edit.textChanged.connect(
            self.on_ollama_model_changed
        )
    ...
    def on_ollama_api_url_changed(self, url: str):
        # Save the Ollama API URL to settings
        self.settings.settings.setValue(Settings.Key.OLLAMA_API_URL.value, url)
        self.settings.settings.sync()
        self.transcription_options_changed.emit(self.transcription_options)
        
    def on_ollama_model_changed(self, model: str):
        # Save the Ollama model name to settings
        self.settings.settings.setValue(Settings.Key.OLLAMA_MODEL.value, model)
        self.settings.settings.sync()
        self.transcription_options_changed.emit(self.transcription_options)
    ...
    def reset_visible_rows(self):
        ...
        # Ollama Whisper API has been removed
        # Hide Ollama fields completely
        self.ollama_api_url_edit.setVisible(False)
        self.ollama_model_edit.setVisible(False)
